...
class Menu:
    ...
    def menu_acciones(self,ventana,tipo):
        ...
        btn_elimin=tk.Button(ventana, text="Eliminar", command=lambda:self.menu_eliminar(ventana, tipo))
    ...
    def menu_mostrar(self, ventana, tipo):
        ...
        if len(registros)>0:
            ...
            #Columnas de tabla
            lbl_title0=tk.Label(marco_texto, text="ID")
            lbl_title0.grid(row=0, column=0, pady=10, padx=5)

            lbl_title1=tk.Label(marco_texto, text="marca")
            lbl_title1.grid(row=0, column=1, pady=10, padx=5)

            lbl_title2=tk.Label(marco_texto, text="color")
            lbl_title2.grid(row=0, column=2, pady=10, padx=5)

            lbl_title3=tk.Label(marco_texto, text="modelo")
            lbl_title3.grid(row=0, column=3, pady=10, padx=5)

            lbl_title4=tk.Label(marco_texto, text="velocidad")
            lbl_title4.grid(row=0, column=4, pady=10, padx=5)

            lbl_title5=tk.Label(marco_texto, text="potencia")
            lbl_title5.grid(row=0, column=5, pady=10, padx=5)

            lbl_title6=tk.Label(marco_texto, text="plazas")
            lbl_title6.grid(row=0, column=6, pady=10, padx=5)
            if tipo=="camiones":
                lbl_title7=tk.Label(marco_texto, text="ejes")
                lbl_title7.grid(row=0, column=7, pady=10, padx=5)

                lbl_title8=tk.Label(marco_texto, text="capacidad de carga")
                lbl_title8.grid(row=0, column=8, pady=10, padx=5)
            elif tipo=="camionetas":
            ...
            for fila in registros:
                #Textos
                lbl_0=tk.Label(marco_texto, text=f"{fila[0]}")
                lbl_0.grid(row=num_vehiculo, column=0, pady=5, padx=5)

                lbl_1=tk.Label(marco_texto, text=f"{fila[1]}")
                lbl_1.grid(row=num_vehiculo, column=1, pady=5, padx=5)

                lbl_2=tk.Label(marco_texto, text=f"{fila[2]}")
                lbl_2.grid(row=num_vehiculo, column=2, pady=5, padx=5)

                lbl_3=tk.Label(marco_texto, text=f"{fila[3]}")
                lbl_3.grid(row=num_vehiculo, column=3, pady=5, padx=5)

                lbl_4=tk.Label(marco_texto, text=f"{fila[4]}")
                lbl_4.grid(row=num_vehiculo, column=4, padx=5, pady=5)

                lbl_5=tk.Label(marco_texto, text=f"{fila[5]}")
                lbl_5.grid(row=num_vehiculo, column=5, padx=5, pady=5)

                lbl_6=tk.Label(marco_texto, text=f"{fila[6]}")
                lbl_6.grid(row=num_vehiculo, column=6, padx=5, pady=5)

                if tipo=="camiones" or tipo=="camionetas":
                    lbl_7=tk.Label(marco_texto, text=f"{fila[7]}")
                    lbl_7.grid(row=num_vehiculo, column=7, padx=5, pady=5)

                    lbl_8=tk.Label(marco_texto, text=f"{fila[8]}")
                    lbl_8.grid(row=num_vehiculo, column=8, padx=5, pady=5)
                num_vehiculo+=1
        else:
    ...
    def menu_actualizar(self, ventana, tipo):
        ...
        txt_id=tk.Entry(ventana, textvariable=ide, validate="key", validatecommand=verificacion_entero)
    ...
    def menu_eliminar(self, ventana, tipo):
        #Borrar pantalla
        self.borrarPantalla(ventana)
        ...
        #Funciones
        def eliminar(ide, tipo):
            dato=coches.funciones.buscar_vehiculo(ide, tipo)
            if dato:
                #Status
                lbl_status.config(text="Se ha encontrado la ID")

                #Botones
                btn_delete=tk.Button(ventana, text="Eliminar", command=lambda:coches.funciones.eliminar_vehiculo(tipo, ide))
                btn_delete.pack(pady=5)
            else:
                lbl_status.config(text="No se encuentra esta ID o un error surgió")
        
        #Titulo
        lbl_titulo=tk.Label(ventana, text=f"... Eliminar un vehículo de tipo {tipo} ...")
        lbl_titulo.pack(pady=[15,25])
        ...
        btn_buscar=tk.Button(marco_botones, text="Buscar", width=12, command=lambda:eliminar(ide.get(), tipo))
        btn_buscar.grid(row=0, column=0, pady=10, padx=20)

        btn_volver=tk.Button(marco_botones, text="Volver", command=lambda:self.menu_acciones(ventana, tipo), width=12)
        btn_volver.grid(row=0, column=1, pady=10, padx=20)

        lbl_status=tk.Label(ventana, text="")
